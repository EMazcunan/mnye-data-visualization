...
Con el método `head()` puedes visualizar las cinco primeras filas de la hoja de datos `penguins` que acabas de crear.

```{python}
penguins.head(5)
```

Y con `tail()` puedes ver las cinco últimas.
...
Indica cuántos valores no nulos tenemos para cada variable. Los valores nulos o perdidos son valores no disponibles, que no han podido registrarse. 
...
Se representan con el símbolo `NaN` (iniciales de _Not A Number_).
